...
def observe_tool_call(
    *, run_id: int | None, trace_id: str | None, step: int | None, tool_name: str, agent_type: str, read_only: bool
):
    with observe_span(
        "agent.tool_call",
        {
...
def record_tool_outcome(
    span: Any, *, success: bool, status: str, duration_ms: int, error_category: str | None = None
) -> None:
    if span is None:
        return
    try:
...
def record_retrieval_result(
    *, run_id: int | None, trace_id: str | None, step: int | None, tool_name: str, result: dict[str, Any]
) -> None:
    """Emit retrieval result shape, never retrieved document text or identifiers."""
    data = result.get("data") if isinstance(result.get("data"), dict) else {}
    candidates = data.get("results") or data.get("items") or data.get("documents") or data.get("chunks") or []
...
def record_state_transition(
    *, run_id: int | None, trace_id: str | None, from_status: str | None, to_status: str | None
) -> None:
